...
{
	int h = open((const char *)filename, O_RDONLY);
	if (h == -1)
		return NULL;
...
static uint64_t inline count_nonzero_pairs(const uint64_t start, const uint64_t end, const uint64_t *const array)
{
	...
	if (block == end_block)
		return _count_nonzero_pairs((array[block] & (UINT64_C(1) << end_offset * 2) - 1) >> start_offset * 2);
	uint64_t pairs = 0;
	if (start_offset != 0)
		pairs += _count_nonzero_pairs(array[block++] >> start_offset * 2);
	while (block < end_block)
		pairs += _count_nonzero_pairs(array[block++]);
	if (end_offset != 0)
		pairs += _count_nonzero_pairs(array[block] & (UINT64_C(1) << end_offset * 2) - 1);
	return pairs;
}

static void inline signature_to_equation(const uint64_t *signature, const uint64_t seed, const int num_variables, int *e)
{
	uint64_t hash[4];
...
uint64_t mph_get_byte_array(const mph *const mph, const unsigned char *const key, const uint64_t len)
{
