...
/// Read the bitstream codes and create an iterator
pub fn decode_node<'a, CR: WebGraphCodesReader, BR: WebgraphBackref>(
    node_id: u64, codes_reader: &mut CR, backrefs: &'a mut BR, min_interval_length: usize,
) -> Result<WebGraphLazyIter<BR::NeighboursIter<'a>>> {
    let mut result = WebGraphLazyIter::default();
