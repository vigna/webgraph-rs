...
///
/// Additionally, each batch is independently memory-mapped from the moment it
/// is created to the end of iteration, so creating too many batches may hit
/// the `/proc/sys/vm/max_map_count` limit and return `ENOMEM: Out of memory`.
///
...
///
/// `SortPairs<(), ()>` has commodity [`SortPairs::new`] and [`SortPairs::push`]
...
///
/// This structure uses an `mmap`ed file under the hood, so watch out for
/// `/proc/sys/vm/max_map_count` on Linux when creating many of them, or you will
/// get `ENOMEM: Out of memory`
