...
pub struct CompFlagsFuzz {
    ...
    pub min_interval_length: u16,
    pub compression_window: u16,
    pub max_ref_count: u16,
}
...
pub fn harness(data: FuzzCase) {
    ...
    let data_be: &[u32] = unsafe { codes_data_be.align_to().1 };
    let data_le: &[u32] = unsafe { codes_data_le.align_to().1 };
    ...
    let mut efb = EliasFanoBuilder::new(
        (data_le.len() * 8 * core::mem::size_of::<u32>()) as u64,
    ...
    for _ in 0..graph.num_nodes() {
        ...
        assert_eq!(succ_be.collect::<Vec<_>>(), succ_le.collect::<Vec<_>>());
