    ...
    for PrefixFree<CODE>
{
    ...
    fn name(&self) -> String {
        match Codes::from_code_const(CODE) {
            Ok(code) => format!("PrefixFree<{}>", code),
...
///
/// ```rust
/// # use webgraph::traits::{Int2Nat, Gamma};
/// // Encode signed values with gamma codes
/// let sd = Int2Nat(Gamma);
/// ```
#[derive(Clone, Copy, Debug)]
pub struct Int2Nat<C>(pub C);
...
mod tests {
    ...
    #[test]
    fn tuple_signed_fixed() {
        let sd = Tuple(FixedWidth::<i8>::with_bits(5), FixedWidth::<i16>::new());
    ...
    #[test]
    fn tuple_nested() {
        let sd = Tuple(Gamma, Tuple(FixedWidth::<u8>::new(), Gamma));
    ...
    #[test]
    fn int2nat_delta_round_trip() {
        let sd = Int2Nat(PrefixFree::<{ code_consts::DELTA }>);
    ...
    #[test]
    fn tuple_int2nat_unsigned() {
        let sd = Tuple(Int2Nat(Gamma), FixedWidth::<u32>::new());
